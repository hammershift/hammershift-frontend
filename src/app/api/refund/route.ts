...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const auction = await db
      .collection("auctions")
      .findOne({ _id: convertedAuctionID });
    const statusAttribute = auction?.attributes.find(
      (attr: { key: string }) => attr.key === "status"
    );
    if (!statusAttribute || statusAttribute.value !== 3) {
      return NextResponse.json(
        { message: "Auction is successful or does not exist" },
        { status: 400 }
      );
    }

    // find wagers related to the unsuccessful auction
    const unsuccessfulWagers = await db
      .collection("wagers")
      .find({ auctionID: convertedAuctionID })
      .toArray();
    ...
    for (let wager of unsuccessfulWagers) {
      ...
      await db
        .collection("users")
        .updateOne({ _id: user._id }, { $set: { balance: updatedBalance } });
    ...
    return NextResponse.json(
      { message: "Refunds processed successfully for unsuccessful auction" },
      { status: 200 }
    );
  } catch (error) {
