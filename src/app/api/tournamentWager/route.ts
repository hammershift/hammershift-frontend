...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!tournament || !tournament.isActive) {
      return NextResponse.json(
        { message: "Invalid or inactive tournament" },
        { status: 400 }
      );
    }
    ...
    if (existingEntry) {
      return NextResponse.json(
        { message: "User has already entered the tournament" },
        { status: 400 }
      );
    }
    ...
    if (userInfo) {
      if (userInfo.balance < buyInAmount) {
        return NextResponse.json(
          { message: "Insufficient funds for buy-in" },
          { status: 400 }
        );
      } else {
    ...
    return NextResponse.json(
      { message: "Tournament wager created successfully" },
      { status: 201 }
    );
  } catch (error) {
