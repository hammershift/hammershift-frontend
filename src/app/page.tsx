...
const LiveGamesCard: React.FC<CarData> = ({ image, year, make, model, description, deadline }) => {
  const playersData = [
    {
      id: 'pl1',
      username: 'user1',
              ...
              {playersData.slice(0, 5).map((item) => {
                return (
                  <div
                    key={item.id}
                    style={{
                      transform: `translate(${-10 + -10 * playersData.slice(0, 5).indexOf(item)}px ,0)`,
                    }}
                  >
      ...
      <div className='section-container tw-grid tw-grid-cols-1 lg:tw-grid-cols-2 tw-pt-16 sm:tw-pt-[120px]  tw-self-center'>
        ...
        <div>
          <p className='tw-mt-8 lg:tw-mt-0'>
            Excepteur sint obcaecat cupiditat non proident culpa. At nos hinc posthac, sitientis piros Afros. Cum sociis natoque penatibus et magnis dis parturient. Quam diu etiam
            furor iste tuus nos eludet?
            <br />
            <br />
            Quam temere in vitiis, legem sancimus haerentia. Phasellus laoreet lorem vel dolor tempus vehicula. Qui ipsorum lingua Celtae, nostra Galli appellantur. Curabitur
            blandit tempus ardua ridiculus sed magna. Tu quoque, Brute, fili mi, nihil timor populi, nihil! Donec sed odio operae, eu vulputate felis rhoncus.
          </p>
      ...
      <div className=' tw-px-4 md:tw-px-16 tw-w-screen 2xl:tw-w-[1440px] tw-pt-8 tw-pb-16'>
        ...
        <section>
          <p className='tw-max-w-[752px] tw-my-12'>
            The excitement of sports betting meets the thrill of car auctions. Car enthusiasts, put your skills to the test by predicting the outcomes of car auctions with
            unmatched precision. Combine knowledge, strategy, and a keen eye for value as the gavel drops and the bidding wars ignite. Join the action by placing wagers on the
            final price the vehicles will go for, which vehicles will command the highest bids, achieve record-breaking prices, or even which ones will surprise the crowd with
            unexpected deals. Sharpen your instincts, analyze market trends, and immerse yourself in the world of rare classics, luxury exotics, and iconic muscle cars.
          </p>
