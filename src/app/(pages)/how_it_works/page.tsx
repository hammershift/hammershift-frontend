export default function HowItWorksPage() {
    return (
        <div className="section-container mx-auto">
            <div className=" mb-4">
                <h2 className="text-4xl font-bold mb-5">
                    How It Works: HammerShift
                </h2>
                <p>
                    Welcome to HammerShift, where the excitement of car auctions
                    meets the thrill of strategic prediction. Here&apos;s a
                    step-by-step guide on how HammerShift transforms the
                    traditional car auction experience into a competitive and
                    engaging platform for car enthusiasts and predictors alike.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 1: Joining HammerShift
                </h3>
                <p>
                    To get started, simply visit our website and sign up.
                    Registration is quick and allows you immediate access to our
                    auction prediction platform. Once registered, you&apos;re
                    ready to dive into the world of auction predictions.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 2: Understanding the Auctions
                </h3>
                <p>
                    HammerShift collaborates with Bring a Trailer (BaT) to bring
                    you live auction feeds. Users can browse through ongoing
                    auctions to select the ones they find most intriguing or
                    where they believe their knowledge of the market will give
                    them an edge in predictions.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 3: Making Predictions
                </h3>
                <p>
                    Once you&apos;ve selected an auction, it&apos;s time to
                    place your wager on the outcome. Predict the final selling
                    price based on your understanding of the car&apos;s value,
                    market trends, and past auction results. This isn&apos;t
                    just a guess; it&apos;s an informed prediction that tests
                    your knowledge and intuition.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 4: Watching the Auction
                </h3>
                <p>
                    As the auction progresses, you can watch in real time as
                    bids are placed. Engage with other members of the
                    HammerShift community through our live chat feature, discuss
                    strategies, and adjust your predictions as the auction
                    unfolds.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 5: Scoring and Rewards
                </h3>
                <p>
                    After the auction concludes, predictions are tallied and
                    winners are determined based on the accuracy of their
                    predictions. Points are awarded not just for accuracy but
                    also for the insight shown in your predictions. Accumulate
                    points to climb the leaderboards and earn rewards.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 6: Learning and Improving
                </h3>
                <p>
                    Whether you win or lose, each auction is a learning
                    opportunity. Review the outcomes and your predictions to
                    refine your strategy for future auctions. HammerShift offers
                    tools and resources to help you analyze past performance and
                    market trends.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Step 7: Community and Competitions
                </h3>
                <p>
                    HammerShift is more than just a prediction platform;
                    it&apos;s a community of car enthusiasts and skilled
                    predictors. Participate in special events, competitions, and
                    community discussions to enhance your experience and connect
                    with like-minded individuals.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Fees and Charges
                </h3>
                <p>
                    There are no fees to join HammerShift. Users can participate
                    in a certain number of free predictions monthly. For more
                    avid users, premium subscriptions are available, offering
                    additional predictions, advanced analytical tools, and
                    exclusive community features.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Customer Support
                </h3>
                <p>
                    Our dedicated support team is available to help with any
                    questions or issues you may encounter. Whether you need help
                    with your account, predictions, or just want to give
                    feedback, we&apos;re here to assist you.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Privacy and Security
                </h3>
                <p>
                    At HammerShift, we prioritize your privacy and security. All
                    personal information is securely stored and protected. We
                    adhere to strict data protection regulations to ensure that
                    your information is safe with us.
                </p>
            </div>
            <div className="mb-4">
                <h3 className="text-2xl font-bold mb-2">
                    Join the Revolution
                </h3>
                <p>
                    HammerShift offers a unique blend of excitement,
                    competition, and learning. Whether you&apos;re a seasoned
                    car enthusiast or new to the world of auctions, HammerShift
                    provides a platform to test your skills, compete with peers,
                    and enjoy the thrill of the auction from anywhere.
                </p>
                <p>
                    Get ready to experience the future of car auctions with
                    HammerShift—where your predictions power your success. Join
                    us today and place yourself in the driver&apos;s seat of
                    auction prediction!
                </p>
            </div>
        </div>
    );
}
