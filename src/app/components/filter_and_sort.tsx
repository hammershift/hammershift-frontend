    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            ...
            const makeFilterButton =
                document.getElementById("filter-make-button");
            const categoryFilterButton = document.getElementById(
                "filter-category-button"
            );
            const eraFilterButton =
                document.getElementById("filter-era-button");
    ...
    return (
        <div className="tw-flex tw-justify-between tw-w-screen tw-my-4 xl:tw-my-8 tw-px-4 md:tw-px-16 2xl:tw-w-[1440px]">
            <div className="left-container-marker tw-flex tw-items-center">
                <div>
                    Live Games{" "}
                    <span className="tw-opacity-50"> {totalAuctions}</span>
                </div>
        ...
        >
            {MakeDropdownContent.map((value) => (
                ...
                >
                    <div
                        onClick={() =>
                            addToFilters(value, "make", filters, router)
                        }
                    >
                        <input
                            type="checkbox"
        ...
        <div className={`tw-px-2 tw-grid tw-grid-cols-${columns}`}>
            {CategoryDropdownContent.map((value) => (
                ...
                >
                    <div
                        onClick={() =>
                            addToFilters(value, "category", filters, router)
                        }
                    >
                        <input
                            type="checkbox"
        ...
        <div className={`tw-px-2 tw-grid tw-grid-cols-${columns}`}>
            {EraDropdownContent.map((value) => (
                ...
                >
                    <div
                        onClick={() =>
                            addToFilters(value, "era", filters, router)
                        }
                    >
                        <input
                            type="checkbox"
            ...
            {LocationDropdownContent.map((value) => {
                return (
                    ...
                    >
                        <div
                            onClick={() =>
                                addToFilters(value, "location", filters, router)
                            }
                        >
                            <input
                                type="checkbox"
