...
const Card: React.FC<any> = ({ image, year, make, model, description, deadline, auction_id, price, object_id }) => {
    ...
    <TimerProvider deadline={new Date(deadline)}>
      <div className='tw-flex tw-flex-col tw-justify-between tw-h-[654px]'>
        <div>
          ...
          <p className='tw-h-[60px] sm:tw-h-[72px] tw-w-full tw-line-clamp-3 tw-overflow-hidden tw-text-[14px] sm:tw-text-[16px]'>{description}</p>
          <div className='tw-flex tw-mt-3'>
            ...
            <div className='tw-text-[#49C742] tw-font-bold'>${new Intl.NumberFormat().format(price)}</div>
          </div>
        ...
        <div>
          <button className='btn-yellow-thin tw-w-full sm:tw-w-auto' onClick={() => router.push(`/auctions/car_view_page/${auction_id}`)}>
            Play Game
          </button>
        </div>
      </div>
    ...
    <TimerProvider deadline={new Date()}>
      <div className='tw-flex tw-flex-col tw-justify-between tw-h-[654px]'>
        <div>
          ...
          <p className='tw-h-[60px] sm:tw-h-[72px] tw-w-full tw-line-clamp-3 tw-overflow-hidden tw-text-[14px] sm:tw-text-[16px]'>{props.description[0]}</p>
          <div className='tw-flex tw-mt-3'>
            <Image src={Dollar} width={20} height={20} alt='dollar' className='tw-w-5 tw-h-5' />
            <div className='tw-px-2 tw-hidden sm:tw-block'>Current Bid:</div>
        ...
        <div>
          <button className='btn-yellow-thin tw-w-full md:tw-w-auto' onClick={() => router.push(`/auctions/car_view_page/${props.auction_id}`)}>
            Play Game
          </button>
        </div>
      </div>
    ...
    <>
      {auctionWagers.length === 0 && (
        <div className='tw-bg-[#172431] tw-p-4 tw-flex tw-gap-2 tw-rounded-[4px] tw-mt-4'>
          <Image src={AvatarFour} width={24} height={24} alt='dollar' className='tw-w-[24px] tw-h-[24px] tw-rounded-full' />
      ...
      {auctionWagers.length !== 0 && (
        <div className='tw-gap-2 tw-bg-[#172431] tw-p-2 sm:tw-p-4 tw-my-4 tw-text-[14px] sm:tw-text-[16px] tw-rounded-[4px]'>
          <div className={`tw-flex tw-flex-col tw-gap-2 ${auctionWagers.length >= 3 && 'tw-mb-3'}`}>
            {auctionWagers.slice(0, 2).map((wager: any) => {
              return (
                <div key={wager.auctionObjectId} className='tw-flex tw-gap-2'>
                  <Image src={wager.user?.image ? wager.user.image : AvatarTwo} width={24} height={24} alt='dollar' className='tw-w-[24px] tw-h-[24px] tw-rounded-full' />
                  <div className='tw-flex tw-text-sm tw-gap-1 tw-items-center'>
                    <div className='tw-text-[#42A0FF]'>{`@${wager.user.username}`}</div>
                    <div>{`wagered $${new Intl.NumberFormat().format(wager.priceGuessed)}`}</div>
                    <div className='tw-text-[#DCE0D9]'>{dayjs(wager.createdAt).fromNow()}</div>
                  </div>
                </div>
              );
            })}
            ...
            <div className='tw-relative tw-flex tw-items-center'>
              ...
              <div className='tw-flex tw-items-center tw-gap-2'>
                <div className=' tw-w-auto tw-hidden xl:tw-flex'>
                  <Image
                    src={(auctionWagers[2] as any).user.image ? (auctionWagers[2] as any).user.image : AvatarTwo}
                    width={32}
                    height={32}
                    alt='avatar'
                  ...
                  />
                  <div className='tw-flex'>
                    {auctionWagers.slice(3, 8).map((wager: any, index: number) => {
                      return (
                        <div
                          key={wager._id}
                          style={{
                        ...
                        >
                          <Image
                            src={wager.user.image ? wager.user.image : AvatarThree}
                            width={32}
                            height={32}
                            alt='avatar'
                            className='tw-w-8 tw-h-8 tw-rounded-full'
                ...
                {auctionWagers.length - 2 == 1 && <div className={`xl:tw-block tw-hidden tw-text-sm`}>{`and ${auctionWagers.length - 2} more player to join`}</div>}
                {auctionWagers.length - 2 == 2 && <div className={`xl:tw-block tw-hidden tw-text-sm -tw-ml-[10px]`}>{`and ${auctionWagers.length - 2} more players to join`}</div>}
                {auctionWagers.length - 2 == 3 && <div className={`xl:tw-block tw-hidden tw-text-sm -tw-ml-[20px]`}>{`and ${auctionWagers.length - 2} more players to join`}</div>}
                {auctionWagers.length - 2 == 4 && <div className={`xl:tw-block tw-hidden tw-text-sm -tw-ml-[30px]`}>{`and ${auctionWagers.length - 2} more players to join`}</div>}
                {auctionWagers.length - 2 >= 5 && <div className={`xl:tw-block tw-hidden tw-text-sm -tw-ml-[40px]`}>{`and ${auctionWagers.length - 2} more players to join`}</div>}
              </div>
              ...
              <div className='tw-ml-1 tw--translate-x-1 tw-block xl:tw-hidden tw-text-sm'>{`${auctionWagers.length - 2} players`}</div>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className=''>
      ...
      <div className='tw-bg-[#1A2C3D] tw-w-auto sm:tw-w-[416px] tw-text-center tw-p-4 tw-rounded-lg tw-mt-12 tw-pt-20'>
        ...
        <div>
          <button className='tw-bg-slate-600 tw-rounded-md tw-h-10 tw-w-full' onClick={() => router.push('/tournament_page')} disabled>
            {/* View Results */}
            Coming Soon
          </button>
        </div>
