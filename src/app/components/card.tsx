    ...
    <TimerProvider deadline={new Date()}>
      <div>
        ...
        <div className='tw-font-bold tw-text-[24px] tw-py-[12px]'>
          {cardData.year} {cardData.brand} {cardData.descritpion}
        </div>
        <p className='tw-h-[60px] sm:tw-h-[72px] tw-w-full tw-text-ellipsis tw-overflow-hidden tw-text-[14px] sm:tw-text-[16px]'>
          This 1981 Ferrari 512 BB is one of 929 carbureted examples produced between 1976 and 1981, and it was imported to the US in July 1981. The car was registered in Oregon
          through the late 2000s and was acquired by the selling dealer in 2022, reportedly from its second owner. It has been refinished in silver over black leather
        </p>
                ...
                {cardData.players.slice(1, 5).map((item) => {
                  return (
                    <div
                      key={item.id}
                      style={{
                        transform: `translate(${-10 + -10 * cardData.players.slice(1, 5).indexOf(item)}px ,0)`,
                      }}
                    >
        ...
        <button className='btn-yellow-thin tw-w-full sm:tw-w-auto' onClick={() => router.push(`/auctions/car_view_page/67232853`)}>
          Play Game
        </button>
      </div>
    </TimerProvider>
                  ...
                  {players.slice(1, 5).map((item) => {
                    return (
                      <div
                        key={item.id}
                        style={{
                          transform: `translate(${-10 + -10 * players.slice(1, 5).indexOf(item)}px ,0)`,
                        }}
                      >
                        <Image src={item.avatar} width={32} height={32} alt='avatar' className='tw-w-8 tw-h-8 tw-rounded-full' style={{ border: '1px solid black' }} />
                      </div>
  ...
  return (
    <div className=''>
      ...
      <div className='tw-bg-[#1A2C3D] tw-w-auto sm:tw-w-[416px] tw-text-center tw-p-4 tw-rounded-lg tw-mt-12 tw-pt-20'>
        ...
        <div>
          <button className='btn-yellow tw-w-full' onClick={() => router.push('/tournament_page')}>
            View Results
          </button>
        </div>
      </div>
