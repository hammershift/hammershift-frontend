      ...
      {isLoggedIn ? (
        <div className=' tw-flex tw-px-4 md:tw-px-16 2xl:tw-px-36 tw-w-screen tw-justify-between tw-py-3'>
          ...
          <div className='tw-hidden sm:tw-flex tw-justify-between tw-items-center tw-w-[136px] md:tw-visible tw-relative'>
            <button id='watchlist-button' className='tw-relative' onClick={() => setDropWatchlist((prev) => !prev)}>
            ...
            <button id='mywagers-button' onClick={() => setDropMyWagers((prev) => !prev)}>
              <Image src={WagersIcon} width={24} height={24} alt='wagers' className='tw-w-[24px] tw-h-[24px]' />
            </button>
            {dropMyWagers && <MyWagersDropdownMenu />}
            <button id='myaccount-button' className='tw-relative' onClick={() => setDropMyAccount((prev) => !prev)}>
  ...
  return (
    <div className='slide-in-top tw-absolute tw-flex-col tw-text-white tw-bg-[#0F1923] tw-p-4 tw-w-full tw-h-full tw-z-50'>
      <div className='tw-relative'>
        <form autoComplete='off' onSubmit={handleSubmit} className='tw-bg-shade-100 tw-flex tw-p-2 tw-rounded tw-mt-8 tw-mb-2'>
          <Image src={MagnifyingGlass} width={15} height={15} alt='magnifying glass' className='tw-w-auto tw-h-auto' />
          <input
      ...
      {!isLoggedIn ? (
        <>
          <Link href='/discover' onClick={closeMenu} className='tw-flex tw-py-2'>
            <div>DISCOVER</div>
          </Link>
          <Link href='/auctions' onClick={closeMenu} className='tw-flex tw-py-2'>
            <div>AUCTIONS</div>
          </Link>
        </>
      ) : (
    ...
    <div className='tw-absolute tw-z-10 tw-right-[112px] tw-top-10 tw-w-[512px] tw-max-h-[784px] tw-overflow-auto tw-bg-[#1A2C3D] tw-rounded tw-py-6 tw-shadow-xl tw-shadow-black'>
      <div className='tw-px-6 tw-flex tw-flex-col tw-gap-4'>
        ...
        <div className='tw-flex'>
          ...
          >
            <div>ACTIVE </div>
            {!isLoading && <span className='tw-px-1 tw-text-sm tw-bg-[#f2ca16] tw-rounded tw-font-bold tw-text-[#0f1923]'>{activeWatchlist.length}</span>}
          </button>
          <button
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWatchlist.length === 0 && (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          ...
          <button onClick={() => router.push('/discover')} className='btn-transparent-white'>
            DISCOVER AUCTIONS
          </button>
        </div>
      )}
      {activeOrCompleted === 'completed' && completedWatchlist.length === 0 && (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>No watchlist in completed</div>
      )}
    </div>
  );
};
  ...
  return (
    <div className='tw-px-6 tw-w-full tw-py-4 tw-border-b-[1px] tw-border-[#253747]'>
      <div className=' tw-w-full tw-py-3 tw-rounded tw-flex tw-items-center tw-gap-6'>
        <Link href={`/auctions/car_view_page/${id}`} className='tw-self-start tw-w-[100px]'>
          <Image src={img} width={100} height={100} alt='wallet icon' className='tw-w-[100px] tw-h-[100px] tw-object-cover tw-rounded-[4px]' />
        </Link>
        <div className='tw-flex tw-flex-col tw-items-start tw-grow tw-max-w-[323px]'>
          <Link href={`/auctions/car_view_page/${id}`} className='tw-self-start'>
...
const MyWagersDropdownMenu = () => {
  ...
  return (
    <div className='tw-absolute tw-z-10 tw-right-[56px] tw-top-10 tw-w-[512px] tw-max-h-[784px] tw-overflow-auto tw-bg-[#1A2C3D] tw-rounded tw-py-6 tw-shadow-xl tw-shadow-black'>
      ...
      {activeOrCompleted === 'completed' && completedWagers.length !== 0 && (
        <div className='tw-w-full'>
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWagers.length === 0 && (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className='tw-'>
      ...
      {activeOrCompleted === 'completed' && completedWagers.length === 0 && (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>No wagers in completed</div>
      )}
    </div>
  );
};
      ...
      <div className=' tw-w-full tw-py-3 tw-rounded tw-flex tw-items-center tw-gap-6'>
        ...
        <div className='tw-flex tw-flex-col tw-items-start tw-grow tw-max-w-[323px]'>
          ...
          <div className='tw-mt-4 tw-w-full tw-p-2 tw-flex tw-gap-4 tw-bg-[#49C74233] tw-rounded'>
            ...
            <div className='tw-text-[#49C742] tw-font-bold tw-text-left'>${new Intl.NumberFormat().format(potential_prize)}</div>
          </div>
        </div>
      </div>
      {/* {type === "Tournament" && (
