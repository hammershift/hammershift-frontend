...
import { getAuctionTransactions, getMyWagers, getMyWatchlist, getTournamentTransactions, getUserPointsAndPlacing, refundWager } from '@/lib/data';
...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      const watchlistActiveButton = document.getElementById('active-watchlist-button');
      const watchlistCompletedButton = document.getElementById('completed-watchlist-button');

      const myWagersButton = document.getElementById('mywagers-button');
      const myWagersActiveButton = document.getElementById('active-mywagers-button');
      const myWagersCompletedButton = document.getElementById('completed-mywagers-button');
      ...
      const claimRefundButtons = document.getElementsByClassName('claim-button') as HTMLCollectionOf<HTMLElement>;

      const isClaimButtonClicked = Array.from(claimRefundButtons).some((button) => button.contains(e.target as Node));
  ...
  return (
    <div>
        ...
        <div className=' tw-flex tw-px-4 md:tw-px-16 tw-w-full tw-justify-between tw-py-3 tw-border-b-[1px] tw-border-b-[#1b252e]'>
          <div className=' tw-flex tw-items-center tw-justify-between'>
            ...
            <Link
              ...
              href='/live'
            >
              <div className={`tw-block tw-mx-4 sm:tw-mx-4 ${pathname === '/live' && 'tw-font-bold tw-border-b-2'}`}>LIVE</div>
            </Link>
            ...
            <Link
              ...
              href='/tournaments'
            >
              <div className={`tw-block tw-mx-4 sm:tw-mx-4 max-sm:tw-hidden ${pathname === '/tournaments' && 'tw-font-bold tw-border-b-2'}`}>TOURNAMENTS</div>
            </Link>
            <div>
              <div className='tw-flex'>
                <Link
                  ...
                  href='/discover'
                >
                  <div className={`tw-block tw-mx-4 sm:tw-mx-4 max-sm:tw-hidden ${pathname === '/discover' && 'tw-font-bold tw-border-b-2'}`}>EXPLORE</div>
                </Link>
                <button
                  onMouseEnter={() => {
                ...
                <Link
                  ...
                  href='/feedback'
                >
                  <div className={`tw-block tw-mx-4 sm:tw-mx-4 ${pathname === '/feedback' && 'tw-font-bold tw-border-b-2'}`}>FEEDBACK</div>
                </Link>
                {navlinkIsOpen && (
                  <div
                    onMouseLeave={() => {
                  ...
                  >
                    <div className='tw-flex tw-flex-col tw-px-1 tw-gap-2' role='menu' aria-orientation='vertical' aria-labelledby='options-menu'>
                      <Link
                        onClick={() => {
                          closeMenu();
                          closeMyAccountMenu();
          ...
          <div className='tw-relative tw-max-w-[535px] tw-w-full tw-hidden lg:tw-block'>
            <form onSubmit={handleSumbit} autoComplete='off' className='tw-w-full tw-flex tw-items-center'>
              <div className={searchBoxDropDown ? 'tw-bg-shade-50 tw-flex tw-py-2 tw-px-3 tw-grow tw-rounded-t' : 'tw-bg-shade-50 tw-flex tw-py-2 tw-px-3 tw-grow tw-rounded'}>
                ...
                {showClearSearchButton && (
                  <Image src={CloseIcon} width={25} height={25} alt='magnifying glass' className='tw-w-[] tw-h-auto tw-cursor-pointer' onClick={clearSearchInputs} />
                )}
              </div>
            </form>
      ...
      {myAccountMenuOpen && <MyAccountMenu closeMyAccountMenu={closeMyAccountMenu} isLoggedIn={isLoggedIn} />}
    </div>
  );
};
  ...
  return (
    <div className='drop-down-custom-height slide-in-top tw-absolute tw-z-50 tw-flex-col tw-text-white tw-bg-[#0F1923] tw-p-4 tw-w-full '>
      <div className='tw-relative'>
        <form autoComplete='off' onSubmit={handleSubmit} className='tw-bg-shade-100 tw-flex tw-justify-between tw-p-2 tw-rounded tw-my-4'>
      ...
      null : (
        <>
          <button onClick={() => setDropWatchlistOrWagers('watchlist')} className={`tw-flex tw-py-2 tw-w-full ${dropWatchlistOrWagers === 'watchlist' && 'tw-font-bold'}`}>
            <Image src={WatchlistIcon} width={24} height={24} alt='watchlist' className='tw-w-[24px] tw-h-[24px]' />
            <div className='tw-ml-4'>MY WATCHLIST</div>
          </button>
          {dropWatchlistOrWagers === 'watchlist' ? <MobileMyWatchlist closeMenu={closeMenu} /> : null}
          <button onClick={() => setDropWatchlistOrWagers('wagers')} className={`tw-flex tw-py-2 tw-w-full ${dropWatchlistOrWagers === 'wagers' && 'tw-font-bold'}`}>
            <Image src={WagersIcon} width={24} height={24} alt='watchlist' className='tw-w-[24px] tw-h-[24px]' />
            <div className='tw-ml-4'>MY WAGERS</div>
          </button>
          {dropWatchlistOrWagers === 'wagers' ? <MobileMyWagers closeMenu={closeMenu} /> : null}
        </>
      )}
...
const MyAccountMenu: React.FC<MyAccountMenuProps> = ({ isLoggedIn, closeMyAccountMenu }) => {
  ...
  return (
    <div className='slide-in-top tw-absolute tw-z-50 tw-flex tw-flex-col tw-text-white tw-bg-[#1A2C3D] tw-p-4 tw-w-full tw-h-auto'>
      ...
      <Link href='/profile' className='tw-p-1.5 hover:tw-bg-white/5 tw-w-full' onClick={closeMyAccountMenu}>
        Profile
      </Link>
      <Link href='/profile' className='tw-p-1.5 hover:tw-bg-white/5 tw-w-full' onClick={closeMyAccountMenu}>
        Settings
...
const MyWatchlistDropdownMenu = () => {
  ...
  const [completedTournamentWatchlist, setCompletedTournamentWatchlist] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchWatchlist = async () => {
      ...
      if (!data.watchlist || data.watchlist.length !== 0) {
        ...
        const completedTournaments = data.tournament_watchlist.filter((watchlist: any) => {
          const auctionDeadline = new Date(watchlist.endTime);
          return auctionDeadline < currentDate;
        });

        const activeTournaments = data.tournament_watchlist.filter((watchlist: any) => {
          const auctionDeadline = new Date(watchlist.endTime);
          return auctionDeadline >= currentDate;
        });
    ...
    <div className='watchlist-menu tw-absolute tw-z-30 tw-right-[112px] tw-top-10 tw-w-[512px] tw-max-h-[784px] tw-overflow-auto tw-bg-[#1A2C3D] tw-rounded tw-pt-6 tw-pb-2 tw-shadow-xl tw-shadow-black'>
      <div className='tw-px-6 tw-flex tw-flex-col tw-gap-4'>
        <div className='tw-flex tw-justify-between'>
          ...
          {(activeWatchlist.length !== 0 || completedWatchlist.length !== 0 || activeTournamentWatchlist.length !== 0 || completedTournamentWatchlist.length !== 0) && (
            <button
              id='myWatchlist-sort'
              type='button'
        ...
        <div className='tw-flex'>
          ...
          >
            <div>ACTIVE </div>
            {!isLoading && (
              <span className='tw-px-1 tw-text-sm tw-bg-[#f2ca16] tw-rounded tw-font-bold tw-text-[#0f1923]'>{activeWatchlist.length + activeTournamentWatchlist.length}</span>
            )}
          </button>
          <button
      ...
      {activeOrCompleted === 'active' && (activeWatchlist.length !== 0 || activeTournamentWatchlist.length !== 0) ? (
        <div className='tw-w-full'>
              ...
              return (
                <div key={watchlist._id}>
                  <TimerProvider deadline={watchlist.endTime}>
                    <MyWatchlistTournamentCard watchlist={watchlist} isActive={true} />
                  </TimerProvider>
                </div>
              );
            })}
      ...
      {activeOrCompleted === 'completed' && (completedWatchlist.length !== 0 || completedTournamentWatchlist.length !== 0) ? (
        <div className='tw-w-full'>
          {wagerSort !== 'TOURNAMENTS' &&
          ...
          {wagerSort !== 'AUCTIONS' &&
            completedTournamentWatchlist.map((watchlist: any) => {
              return (
                <div key={watchlist._id}>
                  <TimerProvider deadline={watchlist.endTime}>
                    <MyWatchlistTournamentCard watchlist={watchlist} isActive={false} />
                  </TimerProvider>
                </div>
              );
            })}
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWatchlist.length === 0 && activeTournamentWatchlist.length === 0 ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          ...
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
            <div className='tw-opacity-70 tw-text-center'>Quam temere in vitiis, legem sancimus haerentia</div>
          </div>
          <button onClick={() => router.push('/auctions')} className='btn-transparent-white'>
            DISCOVER AUCTIONS
          </button>
        </div>
      ) : null}
      {isLoading === false && activeOrCompleted === 'completed' && completedWatchlist.length === 0 && completedTournamentWatchlist.length === 0 ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
            <div className='tw-opacity-70 tw-text-center'>Quam temere in vitiis, legem sancimus haerentia</div>
          </div>
          <button onClick={() => router.push('/auctions')} className='btn-transparent-white'>
            DISCOVER AUCTIONS
      ...
      {isLoading === false && activeOrCompleted === 'completed' && completedWatchlist.length === 0 && wagerSort === 'AUCTIONS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'completed' && completedTournamentWatchlist.length === 0 && wagerSort === 'TOURNAMENTS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWatchlist.length === 0 && wagerSort === 'AUCTIONS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeTournamentWatchlist.length === 0 && wagerSort === 'TOURNAMENTS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
...
export const MyWatchlistTournamentCard = ({ watchlist, isActive, closeMenu }: any) => {
  ...
  return (
    <div className={`sm:tw-px-6 tw-px-5 tw-w-full tw-py-4 tw-border-t-[1px] tw-border-[#253747]`}>
      <div className=' tw-w-full sm:tw-py-3 tw-rounded tw-flex tw-items-center tw-gap-6'>
        ...
        >
          {watchlist.tournamentImages.slice(0, 4).map((image: string, index: number) => {
            return (
              <Image
                key={index}
                src={image}
        ...
        <div className='tw-flex tw-flex-col tw-items-start sm:tw-max-w-[323px] tw-max-w-[230px]'>
          <Link href={`/tournaments/${watchlist.tournamentID}`} className='tw-self-start' onClick={() => closeMenu && closeMenu()}>
            <div className='tw-w-full tw-font-bold sm:tw-text-lg tw-text-base sm:tw-py-1 tw-text-left tw-line-clamp-1'>{watchlist.title}</div>
          </Link>
          {!isActive && <div className='tw-text-xs sm:tw-mb-2 tw-opacity-80'>Ended {formattedDateString}</div>}
...
export const MyWatchlistCard: React.FC<MyWatchlistCardProps> = ({ title, img, current_bid, time_left, id, isActive, closeMenu, index }) => {
  const { days, hours, minutes, seconds } = useTimer();
  let formattedDateString;
  if (time_left) {
    formattedDateString = new Intl.DateTimeFormat('en-US', {
  ...
  return (
    <div className={`sm:tw-px-6 tw-px-5 tw-w-full tw-py-4 ${index === 0 ? '' : 'tw-border-t-[1px] tw-border-[#253747]'}`}>
      <div className=' tw-w-full sm:tw-py-3 tw-rounded tw-flex tw-items-center tw-gap-6'>
        <Link
        ...
        >
          <Image src={img} width={100} height={100} alt='wallet icon' className='sm:tw-w-[100px] tw-w-[50px] tw-h-[50px] sm:tw-h-[100px] tw-object-cover tw-rounded-[4px]' />
        </Link>
        <div className='tw-flex tw-flex-col tw-items-start sm:tw-max-w-[323px] tw-max-w-[230px]'>
          <Link href={`/auctions/car_view_page/${id}`} className='tw-self-start' onClick={() => closeMenu && closeMenu()}>
            <div className='tw-w-full tw-font-bold sm:tw-text-lg tw-text-base sm:tw-py-1 tw-text-left tw-line-clamp-1'>{title}</div>
          </Link>
          {!isActive && <div className='tw-text-xs sm:tw-mb-2 tw-opacity-80'>Ended {formattedDateString}</div>}
          <div className='tw-w-full tw-mt-1 tw-text-sm'>
            {!isActive && (
              <div className='tw-flex tw-items-center tw-gap-2 tw-w-full'>
                <Image src={HammerIcon} width={14} height={14} alt='wallet icon' className='tw-w-[14px] tw-h-[14px]' />
  ...
  useEffect(() => {
    const fetchWagers = async () => {
      ...
      if (!data.wagers || data.wagers.length !== 0) {
        ...
        const completedTournaments = data.tournament_wagers.filter((wager: any) => {
          const auctionDeadline = new Date(wager.tournamentEndTime);
          return auctionDeadline < currentDate;
        });

        const activeTournaments = data.tournament_wagers.filter((wager: any) => {
          const auctionDeadline = new Date(wager.tournamentEndTime);
          return auctionDeadline >= currentDate;
        });
    ...
    <div className='my-wagers-menu tw-absolute tw-z-30 tw-right-[56px] tw-top-10 tw-w-[512px] tw-max-h-[784px] tw-overflow-auto tw-bg-[#1A2C3D] tw-rounded tw-pt-6 tw-pb-2 tw-shadow-xl tw-shadow-black'>
      <div className='tw-px-6 tw-flex tw-flex-col tw-gap-4'>
        <div className='tw-flex tw-justify-between'>
          ...
          {(activeWagers.length !== 0 || completedWagers.length !== 0 || activeTournamentWagers.length !== 0 || completedTournamentWagers.length !== 0) && (
            <button
              id='myWagers-sort'
              type='button'
              className='tw-rounded-sm tw-w-[140px] tw-text-center tw-px-2 tw-py-1.5 tw-text-white-900 tw-shadow-sm tw-bg-[#172431] hover:tw-bg-[#0f1923] tw-truncate'
        ...
        <div className='tw-flex'>
          ...
          >
            <div>ACTIVE </div>
            {!isLoading && (
              <span className='tw-px-1 tw-text-sm tw-bg-[#f2ca16] tw-rounded tw-font-bold tw-text-[#0f1923]'>{activeWagers.length + activeTournamentWagers.length}</span>
            )}
          </button>
          <button
      ...
      {activeOrCompleted === 'active' && (activeWagers.length !== 0 || activeTournamentWagers.length !== 0) ? (
        <div className='tw-w-full'>
          {wagerSort !== 'TOURNAMENTS' &&
      ...
      {activeOrCompleted === 'completed' && (completedWagers.length !== 0 || completedTournamentWagers.length !== 0) ? (
        <div className='tw-w-full'>
          {wagerSort !== 'TOURNAMENTS' &&
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWagers.length === 0 && activeTournamentWagers.length === 0 ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'completed' && completedWagers.length === 0 && completedTournamentWagers.length === 0 ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'completed' && completedWagers.length === 0 && wagerSort === 'AUCTIONS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'completed' && completedTournamentWagers.length === 0 && wagerSort === 'TOURNAMENTS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No completed wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeWagers.length === 0 && wagerSort === 'AUCTIONS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
      ...
      {isLoading === false && activeOrCompleted === 'active' && activeTournamentWagers.length === 0 && wagerSort === 'TOURNAMENTS' ? (
        <div className='tw-px-6 tw-py-16 tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-full tw-gap-4'>
          <Image src={MoneyBag} width={80} height={80} alt='watchlist icon' className='tw-w-[80px] tw-h-[80px]' />
          <div className=''>
            <div className='tw-font-bold tw-text-xl tw-text-center'>No active wagers</div>
...
export const MyWagersTournamentCard = ({ wager, isActive, closeMenu }: any) => {
  ...
  useEffect(() => {
    const fetchPrize = async () => {
      ...
      const totalPrize = 0.88 * transactions.map((transaction: any) => transaction.amount).reduce((accumulator: any, currentValue: any) => accumulator + currentValue, 0);
      setPrize(totalPrize);
    };
    fetchPrize();
  }, []);
    ...
    <div className='sm:tw-px-6 tw-px-5 tw-w-full tw-py-4 tw-border-t-[1px] tw-border-[#253747]'>
      <div className=' tw-w-full sm:tw-py-3 tw-rounded tw-flex tw-items-start tw-gap-6'>
        <Link
          ...
          onClick={() => closeMenu && closeMenu()}
        >
          {wager.tournamentImages.slice(0, 4).map((image: string, index: number) => {
            return (
              <Image
                key={index}
                src={image}
        ...
        <div className='tw-flex tw-flex-col tw-items-start tw-grow tw-w-auto sm:tw-max-w-[323px] tw-max-w-[230px]'>
          <Link href={`/tournaments/${wager._id}`} className='tw-self-start' onClick={() => closeMenu && closeMenu()}>
            <div className={`tw-w-full tw-font-bold sm:tw-text-lg tw-text-base tw-text-left tw-line-clamp-1 ${isActive ? 'sm:tw-mt-[14px]' : 'sm:tw-mt-[5px]'}`}>
          ...
          <div className='tw-w-full tw-mt-1 tw-text-sm'>
            <div className='tw-flex tw-items-center tw-gap-2'>
              <Image src={PodiumIcon} width={14} height={14} alt='wallet icon' className='tw-w-[14px] tw-h-[14px]' />
              <span className='tw-opacity-80'>Place:</span>
              <span className='tw-text-[#F2CA16] tw-font-bold'>
                {isActive ? 'Current: ' : null}
                {pointsAndPlacing.placing ? addNumberSuffix(pointsAndPlacing.placing) : '-'} Place
              </span>
            </div>
            {!isActive && (
              <div className='tw-flex tw-items-center tw-gap-2'>
                <Image src={ThreeStars} width={14} height={14} alt='wallet icon' className='tw-w-[14px] tw-h-[14px]' />
                <span className='tw-opacity-80'>Points:</span> {pointsAndPlacing.totalScore ? new Intl.NumberFormat().format(pointsAndPlacing.totalScore) : '-'} pts. away
              </div>
            )}
            {wager.prize && (
              <div className='sm:tw-mt-4 tw-mt-2 tw-w-full sm:tw-p-2 tw-font-bold tw-p-1 tw-justify-between tw-items-center tw-flex sm:tw-gap-4 tw-bg-[#49c742] tw-text-[#0f1923] tw-rounded sm:tw-text-sm tw-text-xs'>
                <div className='tw-flex tw-gap-2'>
                  <Image src={MoneyBagBlack} width={20} height={20} alt='money bag' className='tw-w-[20px] tw-h-[20px]' />
                  <div>WINNINGS</div>
                </div>
                <div>
                  $
          ...
          {isActive && (
            <div className='sm:tw-mt-[30px] tw-mt-2 tw-w-full sm:tw-p-2 tw-p-1 tw-items-center tw-flex tw-justify-between sm:tw-gap-4 tw-bg-[#49C74233] tw-rounded sm:tw-text-sm tw-text-xs'>
              ...
              <div className='tw-text-[#49C742] tw-font-bold tw-text-left'>{prize ? `$${new Intl.NumberFormat().format(prize)}` : ' --'}</div>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className={`sm:tw-px-6 tw-px-5 tw-w-full tw-py-4 ${index === 0 ? '' : 'tw-border-t-[1px] tw-border-[#253747]'}`}>
      <div className=' tw-w-full sm:tw-py-3 tw-rounded tw-flex tw-items-center tw-gap-6'>
        ...
        <div className='tw-flex tw-flex-col tw-items-start tw-grow tw-w-auto sm:tw-max-w-[323px] tw-max-w-[230px]'>
          <Link href={`/auctions/car_view_page/${id}`} onClick={() => closeMenu && closeMenu()} className='tw-self-start'>
            <div className='tw-w-full tw-font-bold sm:tw-text-lg tw-text-base tw-text-left tw-line-clamp-1'>{title}</div>
          </Link>
          {status === 2 || status === 4 ? <div className='tw-text-xs sm:tw-mb-2 tw-opacity-80'>Ended {formattedDateString}</div> : null}
          <div className='tw-w-full tw-mt-1 tw-text-sm'>
            <div className='tw-flex tw-items-center tw-gap-2'>
              ...
              <span className='tw-text-[#F2CA16] tw-font-bold'>${new Intl.NumberFormat().format(my_wager)}</span>
            </div>
            {prize ? (
              <div className='tw-flex tw-items-center tw-gap-2'>
                <Image src={HammerIcon} width={14} height={14} alt='wallet icon' className='tw-w-[14px] tw-h-[14px]' />
          ...
          {isActive && isRefunded && (
            <div className='sm:tw-mt-4 tw-mt-2 tw-w-full sm:tw-p-2 tw-p-1 tw-items-center tw-flex sm:tw-gap-4 tw-gap-2 tw-bg-[#4b2330] tw-rounded sm:tw-text-sm tw-text-xs'>
              <div className='tw-text-[#f92f60] tw-font-bold tw-text-left tw-grow-[1]'>❌ AUCTION WAGER</div>

              <button disabled className='tw-bg-[white] tw-text-[black] tw-text-[12px] tw-font-bold tw-text-left tw-px-2 tw-rounded-sm'>
                REFUNDED
              </button>
            </div>
          )}
          {status === 3 && (
            <>
              <div className='tw-flex tw-items-center tw-gap-2 tw-w-full tw-text-sm'>
                ...
                <span className='tw-text-[#f92f60] tw-font-bold'>${new Intl.NumberFormat().format(wagerAmount)}</span>
              </div>
              <div className='sm:tw-mt-4 tw-mt-2 tw-w-full sm:tw-p-2 tw-p-1 tw-items-center tw-flex sm:tw-gap-4 tw-gap-2 tw-bg-[#4b2330] tw-rounded sm:tw-text-sm tw-text-xs'>
                <div className='tw-text-[#f92f60] tw-font-bold tw-text-left tw-grow-[1]'>
                  ❌ UNSUCCESSFUL <span className='tw-hidden sm:tw-inline-block'>AUCTION</span>
                </div>
                {refunded ? (
                  <button disabled className='tw-bg-[white] tw-text-[black] tw-text-[12px] tw-font-bold tw-text-left tw-px-2 tw-rounded-sm'>
                    REFUNDED
            ...
            {wagerSort !== 'AUCTIONS' &&
              activeTournamentWatchlist.map((watchlist: any) => {
                return (
                  <div key={watchlist._id}>
                    <TimerProvider deadline={watchlist.endTime}>
                      <MyWatchlistTournamentCard watchlist={watchlist} isActive={true} closeMenu={closeMenu} />
                    </TimerProvider>
                  </div>
                );
              })}
            ...
            {wagerSort !== 'AUCTIONS' &&
              completedTournamentWatchlist.map((watchlist: any) => {
                return (
                  <div key={watchlist._id}>
                    <TimerProvider deadline={watchlist.endTime}>
                      <MyWatchlistTournamentCard watchlist={watchlist} isActive={false} closeMenu={closeMenu} />
                    </TimerProvider>
                  </div>
                );
              })}
    ...
    <div className='tw-relative'>
      ...
      <div className='tw-flex'>
        ...
        >
          ACTIVE
          {!isLoading && (
            <span className='tw-ml-1 tw-px-1 tw-text-xs tw-bg-[#f2ca16] tw-rounded tw-font-bold tw-text-[#0f1923]'>{activeWagers.length + activeTournamentWagers.length}</span>
          )}
        </button>
        <button
          onClick={() => setActiveOrCompleted('completed')}
            ...
            {wagerSort !== 'AUCTIONS' &&
              activeTournamentWagers.map((wager: any) => {
                return (
                  <div key={wager._id}>
                    <TimerProvider deadline={wager.tournamentEndTime}>
                      <MyWagersTournamentCard wager={wager} isActive={true} closeMenu={closeMenu} />
                    </TimerProvider>
                  </div>
                );
              })}
            ...
            {wagerSort !== 'AUCTIONS' &&
              completedTournamentWagers.map((wager: any) => {
                return (
                  <div key={wager._id}>
                    <TimerProvider deadline={wager.tournamentEndTime}>
                      <MyWagersTournamentCard wager={wager} isActive={false} closeMenu={closeMenu} />
                    </TimerProvider>
                  </div>
                );
              })}
