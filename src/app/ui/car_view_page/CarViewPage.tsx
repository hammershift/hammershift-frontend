        ...
        <div className='right-section-marker'>
          <div className='top-section-marker tw-flex tw-flex-col md:tw-flex-row tw-justify-between'>
            ...
            <div className='tw-flex'>
              ...
              <span className='tw-opacity-80'>
                Time Left: <span className='tw-font-bold tw-text-[#C2451E]'>{`${timerValues.days}:${timerValues.hours}:${timerValues.minutes}:${timerValues.seconds}`}</span>
              </span>
            </div>
          </div>
  ...
  const updateWatchlist = async (add: boolean) => {
    ...
    try {
      const response = await fetch('/api/myWatchlist', {
        ...
        body: JSON.stringify({ auctionID, action: add ? 'add' : 'remove' }),
      });
      ...
      {isLoading ? (
        ...
        <div>
          <div className='tw-flex tw-gap-4'>
            <button
              className={`btn-transparent-white tw-flex tw-items-center ${isWatching ? 'tw-border-green-500' : 'tw-border-transparent'} tw-transition-all`}
              onClick={handleWatchClick}
            >
              <Image
                src={isWatching ? CheckIconGreen : WatchListIcon}
                width={20}
                height={20}
                alt={isWatching ? 'Checked' : 'Watch'}
                className={`tw-w-5 tw-h-5 tw-mr-2 ${isWatching ? 'scale-animation is-watching' : 'scale-animation'}`}
              />
              {isWatching ? 'WATCHED' : 'WATCH'}
            </button>
            {alreadyWagered ? (
              <button type='button' disabled className='tw-flex tw-items-center tw-px-3.5 tw-py-2.5 tw-gap-2 tw-text-[#0f1923] tw-bg-white tw-font-bold tw-rounded'>
                WAGERED <Image src={CheckIcon} alt='' className='tw-border-2 tw-border-[#0f1923] tw-rounded-full tw-p-[1.5px] tw-w-5 tw-h-5 black-check-filter' />
              </button>
            ) : (
...
export const ArticleSection: React.FC<ArticleSectionProps> = ({ description, images_list, toggleWagerModal, alreadyWagered }) => {
    ...
    <div className='tw-flex tw-mt-8 tw-text-[14px]'>
      ...
      <div className='tw-ml-4'>
        ...
        <div className='tw-text-[#42A0FF] tw-mt-3 tw-flex'>
          <Image src={CornerDownRight} width={16} height={16} alt='camera plus' className='tw-w-4 tw-h-4 tw-mr-2 ' />1 Replay
        </div>
      </div>
    </div>
  );
...
export const WagersSection: React.FC<WagersSectionProps> = ({ players_num, wagers, toggleWagerModal, alreadyWagered }) => {
  const { data: session } = useSession();
  const router = useRouter();
    ...
    <div className='tw-mt-8 lg:tw-mt-16 tw-bg-[#172431] tw-p-6'>
      ...
      <div>
        ...
        <div className='tw-flex tw-justify-between tw-py-2'>
          ...
          <Link href={'/'}>
            <span className='tw-underline tw-underline-offset-4'>{images_list.length + 1} photos</span>
          </Link>
        </div>
