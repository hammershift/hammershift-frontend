...
import { getAuctionsByTournamentId, getLimitedTournaments, getTournamentPointsByTournamentId } from '@/lib/data';
...
export const TournamentButtons: React.FC<TournamentButtonsI> = ({
  ...
  useEffect(() => {
    const storedWatchStatus = localStorage.getItem(`watchStatus_${tournamentID}`);
  ...
  return (
    <div className='tw-flex tw-gap-4'>
      <button className={`btn-transparent-white tw-flex tw-items-center tw-transition-all`} onClick={handleWatchClick}>
        <Image
          src={WatchListIcon}
          width={20}
          height={20}
          alt={isWatching ? 'Checked' : 'Watch'}
          className={`tw-w-5 tw-h-5 tw-mr-2 ${isWatching ? 'scale-animation is-watching' : 'scale-animation'}`}
        />
      ...
      {tournamentEnded ? (
        ...
        <button disabled className='tw-flex tw-items-center tw-px-3.5 tw-py-2.5 tw-gap-2 tw-text-[#0f1923] tw-bg-white tw-font-bold tw-rounded'>
          Buy-in period has ended
        </button>
      ) : alreadyJoined ? (
        <button type='button' disabled className='tw-flex tw-items-center tw-px-3.5 tw-py-2.5 tw-gap-2 tw-text-[#0f1923] tw-bg-white tw-font-bold tw-rounded'>
          JOINED <Image src={CheckIcon} alt='' className='tw-border-2 tw-border-[#0f1923] tw-rounded-full tw-p-[1.5px] tw-w-5 tw-h-5 black-check-filter' />
        </button>
      ) : (
        <button
...
export const TitleSingleCarContainer: React.FC<TitleSingleCarContainerProps> = ({
  year,
  make,
  model,
  current_bid,
    ...
    <div className=' tw-flex tw-flex-col tw-flex-grow tw-w-auto'>
        ...
        <div className='tw-w-[280px]'>
          <div className='tw-flex tw-items-center'>
            ...
            <div className='tw-opacity-80 tw-flex'>
              Current Bid:
              <span className='tw-text-[#49C742] tw-font-bold tw-ml-2'>{`$ ${String(current_bid)}`}</span>
            ...
            <div className='tw-flex tw-w-[270px] tw-items-center'>
              ...
              <span className='tw-opacity-80'>
                ...
                {new Date(deadline) < new Date() ? (
                  <span className='tw-font-bold tw-text-[#C2451E]'>Ended {dayjs(deadline).fromNow()}</span>
                ) : (
      ...
      <div className='tw-opacity-80 md:tw-flex md:tw-mt-1'>
        <div className='tw-flex tw-gap-2 tw-items-center tw-w-full tw-max-w-[280px]'>
          <Image src={CommentsIcon} width={20} height={20} alt='calendar' className='tw-w-5 tw-h-5 tw-text-white' />
          <div>
            Comments: <span className='tw-font-bold'>{comments ? new Intl.NumberFormat().format(comments) : '--'} </span>
          </div>
        </div>
        <div className='tw-flex tw-gap-2 tw-items-center  tw-w-full tw-max-w-[270px]'>
          <Image src={EyeIcon} width={20} height={20} alt='calendar' className='tw-w-5 tw-h-5 tw-text-white' />
          <div>
            Views: <span className='tw-font-bold'>{views ? new Intl.NumberFormat().format(views) : '--'}</span>
          </div>
        </div>
        <div className='tw-flex tw-gap-2 tw-items-center'>
          <Image src={TelescopeIcon} width={20} height={20} alt='calendar' className='tw-w-5 tw-h-5 tw-text-white' />
          <div>
            Watchers: <span className='tw-font-bold'>{watchers ? new Intl.NumberFormat().format(watchers) : '--'}</span>
          </div>
        </div>
      </div>
    </div>
...
export const TitleTournamentsList: React.FC<Tournaments> = ({ title, cars, pot, endTime, tournamentEndTime }) => {
  ...
  return (
    <div className=' tw-flex tw-flex-col tw-flex-grow tw-w-auto'>
      ...
      <div className='title-section-marker tw-flex tw-text-3xl md:tw-text-5xl tw-font-bold'>{title}</div>
      <div className=' tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-mt-6'>
        ...
        <div className='tw-flex'>
          ...
          <span className='tw-opacity-80'>
            Buy-in Ends:{' '}
            {buyInEnded ? (
              <span className='tw-font-bold tw-text-[#C2451E]'>Ended {dayjs(endTime).fromNow()}</span>
            ) : (
        ...
        <div className='tw-flex tw-mt-0 md:tw-mt-1'>
          ...
          <span className='tw-opacity-80'>
            Tournament Ends:{' '}
            {tournamentEnded ? <span className='tw-font-bold'>Ended {dayjs(tournamentEndTime).fromNow()}</span> : <span className='tw-font-bold'>{formattedEndTime}</span>}
          </span>
        </div>
        <div className='tw-flex'>
          <div>
...
export const TournamentsList: React.FC<TournamentListI> = ({ buyInFee, toggleTournamentWagerModal, auctionData, alreadyJoined, tournamentID, tournamentEnded }) => {
  const router = useRouter();
    ...
    <div className='tw-mt-8 md:tw-mt-16'>
      ...
      <div className='tw-flex tw-flex-col'>
        {auctionData.map((item, index) => (
          <Link href={`/tournaments/${tournamentID}/${item.auction_id}/`} key={index} className='hover:tw-cursor-pointer'>
...
export const PhotosLayout: React.FC<PhotosLayoutProps> = ({ images_list, img, showCarImageModal, toggleModal }) => {
  return (
    <div className=' tw-my-8'>
      <CarImageModal isOpen={showCarImageModal} onClose={toggleModal} image={images_list} />
      <img onClick={toggleModal} src={img} width={832} height={520} alt='car' className='tw-w-full tw-max-h-[520px] tw-object-cover tw-rounded tw-aspect-auto tw-cursor-pointer' />
      <div className='tw-grid tw-grid-cols-4 tw-gap-2 tw-mt-2 tw-w-full tw-h-auto'>
        <img src={images_list[0].src} width={202} height={120} alt='car' className='tw-w-full tw-max-h-[120px] tw-object-cover tw-rounded tw-aspect-auto' />
        <img src={images_list[1].src} width={202} height={120} alt='car' className='tw-w-full tw-max-h-[120px] tw-object-cover tw-rounded tw-aspect-auto' />
        <img src={images_list[2].src} width={202} height={120} alt='car' className='tw-w-full tw-max-h-[120px] tw-object-cover tw-rounded tw-aspect-auto' />
        <div className='tw-relative tw-cursor-pointer' onClick={toggleModal}>
          <img src={images_list[3].src} width={202} height={120} alt='car' className='tw-w-full tw-max-h-[120px] tw-object-cover tw-opacity-40 tw-rounded tw-aspect-auto' />
...
export const ArticleSection: React.FC<ArticleSectionProps> = ({ toggleTournamentWagerModal, description, images_list, alreadyJoined, tournamentEnded }) => {
  ...
  return (
    <div className='tw-flex tw-flex-col tw-mt-8 md:tw-mt-16 tw-w-full'>
      ...
      {alreadyJoined || tournamentEnded ? null : (
        <button className='btn-yellow tw-mt-3' onClick={toggleTournamentWagerModal}>
          PLACE MY WAGER
        </button>
      )}
    </div>
    ...
    <div>
      <div className='tw-p-6 tw-bg-[#172431] tw-rounded-lg'>
        ...
        <div className='tw-my-4'>
          Get more points the closer you are to the hammer price of a curated set of car auctions. Duis anim adipisicing minim nisi elit quis. Cillum ullamco qui dolore non
...
export const TournamentWagersSection: React.FC<TournamentWagerSectionI> = ({ toggleTournamentWagerModal, tournamentWagers, alreadyJoined, tournamentEnded, auctionID }) => {
  const { data: session } = useSession();
      ...
      <div className='tw-relative tw-pb-8 sm:tw-pb-0 tw-min-h-[180px]'>
        <div className='tw-p-6 tw-w-full tw-h-auto'>
          <div className='tw-mb-6'>
            ...
            <div className='tw-text-[14px]'>{tournamentWagers.length ? tournamentWagers.length : 0} Players</div>
          </div>
          <div>
            {morePlayers &&
              tournamentWagers.map((wager) => {
                return (
                  <div key={wager._id} className='tw-flex tw-items-center tw-py-2'>
                    <div className='tw-flex'>
                      <Image
                      ...
                      <div className='tw-text-sm '>
                        <div className='tw-font-bold'>{session?.user.id === wager.user._id ? 'You' : wager.user.username}</div>
                        <div className='tw-opacity-50'>{`Joined ${dayjs(wager.createdAt).fromNow()}`}</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          {tournamentWagers.length > 4 ? (
            morePlayers ? (
              <button className='btn-transparent-white tw-w-full tw-mt-2' onClick={() => setMorePlayers(false)}>
                Less Players...
              </button>
            ) : (
              <button className='btn-transparent-white tw-w-full tw-mt-2' onClick={() => setMorePlayers(true)}>
                More Players...
              </button>
            )
          ) : null}
          ...
          {alreadyJoined || tournamentEnded ? null : (
            <button
              ...
              className={`btn-yellow tw-w-full ${tournamentWagers.length > 4 ? 'tw-mt-2' : 'tw-mt-6'}`}
            >
              JOIN TOURNAMENT
            </button>
          )}
        ...
        <div className='tw-absolute tw-top-0 tw-bottom-0 tw-z-[-1] tw-w-full'>
          <Image src={TransitionPattern} width={288} height={356} alt='pattern' className='tw-w-full tw-h-auto tw-rounded-lg tw-mr-1 tw-object-cover' />
    ...
    <div className='tw-bg-[#172431] tw-p-6 tw-rounded-lg'>
      ...
      <div>
        ...
        <div className='tw-flex tw-justify-between tw-py-2'>
          ...
          <div className='tw-flex tw-items-center '>
            {props.website}
            <Image src={BringATrailerLogo} width={32} height={32} alt='bring a trailer logo' className='tw-w-[32px] tw-h-[32px] tw-ml-2' />
          </div>
        </div>
        <hr className='tw-border-white tw-opacity-5' />
        <div className='tw-flex tw-justify-between tw-py-2'>
        ...
        <div className='tw-flex tw-justify-between tw-py-2'>
          ...
          <Link href={'/'}>
            <span className='tw-underline tw-underline-offset-4'>88 photos</span>
          </Link>
        </div>
...
export const TournamentsYouMightLike = () => {
  ...
  const [tournamentPointsData, setTournamentPointsData] = useState<TournamentPoints[]>([]);
  ...
  useEffect(() => {
    const fetchTournamentsData = async () => {
      try {
        ...
        const tournamentPointsPromises = tournamentsArray.map((tournament: { _id: string }) => getTournamentPointsByTournamentId(tournament._id, playerLimit));
        const tournamentPointsArray = await Promise.all(tournamentPointsPromises);
    ...
    const fetchAuctionData = async () => {
      try {
        ...
        for (const tournament of tournamentsData) {
          const auctionDataForTournament = await getAuctionsByTournamentId(tournament._id);
      ...
      <section className='tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 lg:tw-grid-cols-3 tw-gap-8 tw-mt-8'>
        {tournamentsData &&
          tournamentsData.slice(0, 3).map((tournament, index) => {
            const imagesForTournament = auctionData[tournament._id]?.map((auction) => auction.image) || [];
    ...
    <div className='tw-bg-[#156cc3] tw-p-6 tw-rounded-lg'>
      ...
      <div>
        {winners.map((winner: any, index: number) => {
          return (
            <div key={winner.userID} className='tw-flex tw-justify-between tw-items-center tw-py-2'>
              <div className='tw-flex tw-gap-4 tw-items-center'>
                <div className='tw-text-lg tw-opacity-30'>{winner.rank}</div>
                <Image src={winner.userImage ? winner.userImage : AvatarOne} width={44} height={44} alt='arrow down' className='tw-w-[44px] tw-h-[44px] tw-rounded-full' />
                <div>
                  <div className='tw-text-sm tw-font-bold'>{session?.user.id === winner.userID ? 'You' : winner.username} 🎉</div>
        ...
        <div className='tw-flex tw-gap-6 tw-py-4 tw-px-6 tw-items-center tw-bg-[#2c7bc9] tw-rounded-[10px] tw-mt-6'>
          ...
          <div>
            ...
            <div className='tw-text-sm tw-opacity-70 tw-leading-5'>
              ...
              in this game. The amount has been added to your wallet. Hope to see you in more games.
            </div>
          </div>
        </div>
      )}
        ...
        <div className='tw-p-6 tw-w-full tw-h-auto'>
          ...
          <div>
            {sortedTournamentPointsData &&
              sortedTournamentPointsData.map((item: any, index: number) => {
                return (
                  <div key={item._id} className='tw-flex tw-justify-between tw-items-center tw-py-2'>
                    <div className='tw-flex tw-justify-between tw-items-center tw-gap-4'>
                      <div className='tw-text-lg tw-opacity-30'>{index + 1}</div>
                    ...
                    <div className='tw-w-auto tw-px-6 tw-py-1 tw-text-sm tw-font-bold tw-text-black tw-h-auto tw-bg-yellow-400 tw-rounded-md'>{item.totalScore} pts.</div>
                  </div>
                );
              })}
          </div>
